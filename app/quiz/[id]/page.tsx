...
export default function DynamicQuiz() {
  ...
  const handleSubmit = async () => {
    ...
    if (correctCount < 2) {
      alert(`Oops! You got ${correctCount} correct. You need to get at least 2 correct answers to pass. Let's watch the lesson video again to learn more! 📚`);
      return;
    }
    ...
    if (userData && questions.length > 0) {
      ...
      /* Append game-unlock notice for all children/students */
      if (unlockedGame) {
        ...
        <div className="max-w-4xl mx-auto w-full p-6 relative z-10">
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8 mb-10 border border-green-100">
            ...
            {!submitted ? (
              ...
              <div className={`mt-10 p-8 rounded-3xl border-2 text-center animate-in fade-in zoom-in duration-500 ${
                correctCount < 2 ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"
              }`}>
                <h2 className={`text-3xl font-bold mb-2 ${correctCount < 2 ? "text-red-800" : "text-green-800"}`}>
                  {correctCount < 2 ? "Keep Trying! 📚" : "Quiz Finished! 🎉"}
                </h2>
                <p className={`text-xl mb-6 ${correctCount < 2 ? "text-red-700" : "text-green-700"}`}>
                  You got <span className="font-black text-4xl">{correctCount}</span> out of {questions.length} correct!
                  {correctCount < 2 && " (You need at least 2 correct answers to pass)"}
                </p>
                
                {correctCount < 2 ? (
                  <button
                    onClick={() => {
                      const QUIZ_VIDEO_URLS: Record<string, string> = {
                        video1: "https://www.youtube.com/embed/FpOWG4GDvx4",
                        video2: "https://www.youtube.com/embed/V_1vpEEnXW0",
                        video3: "https://www.youtube.com/embed/aLY46g18hWk",
                        video4: "https://www.youtube.com/embed/tNbTppAbEVc",
                        video5: "https://www.youtube.com/embed/AltruHFIBAQ",
                        video6: "https://www.youtube.com/embed/xpAnLXc_bIU",
                      };
                      const targetUrl = QUIZ_VIDEO_URLS[quizId]
                        ? `/lesson-page?video=${encodeURIComponent(QUIZ_VIDEO_URLS[quizId])}`
                        : "/lesson-page";
                      router.push(targetUrl);
                    }}
                    className="px-10 py-4 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-bold text-lg transition shadow-md"
                  >
                    Watch Lesson Again 📺
                  </button>
                ) : correctCount >= 3 && quizId !== "video6" ? (
                  <button
                    onClick={() => {
                      const QUIZ_NEXT_VIDEO_URLS: Record<string, string> = {
                        video1: "https://www.youtube.com/embed/V_1vpEEnXW0",
                        video2: "https://www.youtube.com/embed/aLY46g18hWk",
                        video3: "https://www.youtube.com/embed/tNbTppAbEVc",
                        video4: "https://www.youtube.com/embed/AltruHFIBAQ",
                        video5: "https://www.youtube.com/embed/xpAnLXc_bIU",
                      };
                      const targetUrl = QUIZ_NEXT_VIDEO_URLS[quizId]
                        ? `/lesson-page?video=${encodeURIComponent(QUIZ_NEXT_VIDEO_URLS[quizId])}`
                        : "/lesson-page";
                      router.push(targetUrl);
                    }}
                    className="px-10 py-4 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-lg transition shadow-md animate-bounce"
                  >
                    Next Lesson 🚀
                  </button>
                ) : (
                  <button
                    ...
                    className="px-10 py-4 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-lg transition shadow-md"
                  >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[10000] p-4 animate-fadeIn">
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            {[...Array(25)].map((_, i) => (
              <div
                key={i}
                className="absolute w-3 h-3 rounded-full opacity-75 animate-confettiFall"
                style={{
                  backgroundColor: ["#FFD700", "#FF4500", "#32CD32", "#1E90FF", "#FF69B4"][i % 5],
          ...
          <div className="bg-gradient-to-br from-white to-[#F5FFF9] rounded-[40px] border-4 border-green-500 p-8 max-w-[550px] w-full text-center relative shadow-2xl overflow-hidden animate-zoomIn">
            {/* Mascot Image with bounce and golden shine */}
            ...
            {/* Title */}
           <h2 className="text-4xl font-extrabold text-green-700 mb-4 font-fredoka flex items-center justify-center gap-2">
  🏆 Amazing Job! 🏆
</h2>

{/* English celebratory text */}
<p className="text-xl text-gray-700 font-bold mb-8 leading-relaxed">
  You have successfully completed the final quiz! 🥳✨
  <br />
  You are now a <span className="text-green-600 font-black text-2xl">true Environmental Hero 🌳🌍</span>
  <br />
  Your beautiful tree is very proud of you, and we are proud of your amazing achievement too! 🌱
</p>

            {/* XP and Gold Badge */}
            <div className="bg-yellow-100/80 border-2 border-yellow-300 rounded-3xl p-4 mb-8 inline-flex items-center gap-3 shadow-inner">
              <span className="text-3xl">🌟</span>
              <div className="text-left">
                <p className="text-xs font-bold text-yellow-800 uppercase tracking-wider">ECO HERO LEVEL UNLOCKED</p>
                <p className="text-lg font-black text-yellow-700">+{correctCount * 10} XP Added to Tree</p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                ...
                className="px-8 py-4 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-lg transition-all shadow-md active:scale-95 cursor-pointer"
              >
                Go to Dashboard 🌳
              </button>
              <button
                onClick={() => router.push("/lesson-page")}
                className="px-8 py-4 bg-green-50 text-green-700 border-2 border-green-200 hover:bg-green-100 rounded-2xl font-bold text-lg transition-all shadow-sm active:scale-95 cursor-pointer"
              >
                Back to Lessons 📚
              </button>
            </div>
