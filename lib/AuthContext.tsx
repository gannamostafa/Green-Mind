...
export interface ChildProgress {
  ...
  gamesProgress: { puzzle: number; memory: number; sundew_vally: number };
  treeLevel: number;
...
export const GAME_KEYS: string[] = [
  "totalMoney",
  "globalHints",
  "sundew_valley_save"
];
for (let i = 1; i <= 10; i++) {
  GAME_KEYS.push(`level${i}Completed`);
  GAME_KEYS.push(`level${i}Score`);
  GAME_KEYS.push(`level${i}Stars`);
}
...
export const AuthProvider = ({ children: reactChildren }: { children: React.ReactNode }) => {
  ...
  const prevChildRef = useRef<{ userId: string; childId: string } | null>(null);

  /* ---------- LocalStorage Isolation for Children Games ---------- */
  useEffect(() => {
    if (loading) return;
    ...
    // Backup the previous child's game data
    if (prevChildRef.current) {
      const { userId: prevUserId, childId: prevChildId } = prevChildRef.current;
      if (prevUserId !== currentUserId || prevChildId !== currentChildId) {
        const backupData: Record<string, string | null> = {};
        GAME_KEYS.forEach((key) => {
          backupData[key] = localStorage.getItem(key);
        });
        localStorage.setItem(`gameBackup_${prevUserId}_${prevChildId}`, JSON.stringify(backupData));
        
        // Clear global game keys
        GAME_KEYS.forEach((key) => {
    ...
    // Restore the current child's game data
    if (currentUserId && currentChildId) {
      const backupStr = localStorage.getItem(`gameBackup_${currentUserId}_${currentChildId}`);
      if (backupStr) {
        try {
          ...
          console.error("Error restoring game backup:", e);
        }
      } else {
        GAME_KEYS.forEach((key) => {
          localStorage.removeItem(key);
      ...
      prevChildRef.current = { userId: currentUserId, childId: currentChildId };
    } else {
  ...
  const addChild = async (childData: Partial<ChildData>) => {
    ...
    try {
      ...
      // Set new child as active (index = updated.length - 1)
      await setDoc(userRef, { children: updated, activeChildIndex: updated.length - 1 }, { merge: true });
      setChildrenList(updated);
  ...
  const switchChild = async (index: number) => {
    ...
    // If the child lacks a progress object, initialise it with defaults
    if (!selectedChild.progress) {
      const userRef = doc(db, "users", user.uid);
      const updatedChildren = [...childrenList];
      updatedChildren[index] = { ...selectedChild, progress: { ...defaultProgress } };
      await setDoc(userRef, { children: updatedChildren }, { merge: true });
      setChildrenList(updatedChildren);
    }
    // Update active child index in Firestore and local state
    const userRef = doc(db, "users", user.uid);
    await setDoc(userRef, { activeChildIndex: index }, { merge: true });
    setActiveChildIndex(index);
    // ---- Reset localStorage for the newly active child ----
    // Games progress defaults (0%)
    const gpKey = `gamesProgress_${user.uid}_${selectedChild.id}`;
    localStorage.setItem(gpKey, JSON.stringify({ puzzle: 0, memory: 0, sundew_vally: 0 }));
    // Tree level default (1)
    const treeKey = `treeLevel_${user.uid}_${selectedChild.id}`;
    localStorage.setItem(treeKey, "1");
  };
