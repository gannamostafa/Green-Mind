...
export default function GamesPage() {
  const { user, userData, loading, activeChildIndex, updateUserData } = useAuth();
  ...
  // Sync game progress from local storage completed levels to Firestore
  const syncGamesProgress = async () => {
    if (!userData) return;
    ...
    // Sundew Valley progress from save state
    ...
    if (!isFarmLocked) {
      ...
      if (sundewSave) {
        try {
          ...
          const questsCompleted = data.world?.quests?.completed?.length || 0;
    ...
    if (hasChanges) {
      ...
      console.log("Synced game progress from iframe completion!", { puzzlePercent, memoryPercent, matchingPercent });
    }
  };

  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // Close game message (from iframe)
      if (event.data === "closeGame") {
        setActiveGame(null);
        syncGamesProgress().catch(e => console.error("Error syncing on close:", e));
        return;
      }

      // Level-complete messages from embedded games: { type: 'levelComplete', game: 'puzzle' | 'memory' }
      try {
        const payload = typeof event.data === "string" ? JSON.parse(event.data) : event.data;
        if (payload && payload.type === "levelComplete" && (payload.game === "puzzle" || payload.game === "memory") && userData) {
          const currentProgress = userData.progress || {
            scansCount: 0,
            lessonsCompleted: [],
            quizScores: {},
          ...
          const key = payload.game as "puzzle" | "memory" | "sundew_vally";
          const currentValue = currentProgress.gamesProgress?.[key] || 0;
          ...
          updateUserData({
            progress: {
              ...currentProgress,
              gamesProgress: {
                ...currentProgress.gamesProgress,
                [key]: newValue,
              },
            },
          }).catch(err => console.error("Error updating progress on levelComplete:", err));
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (!loading && user && userData) {
      syncGamesProgress().catch(e => console.error("Error on mount sync:", e));
    }
  }, [loading, user, userData?.id]);
  ...
  const handleGameClick = async (game: typeof games[0]) => {
    if (isGameLocked(game.id)) {
      ...
      if (game.id === "farm" && userData) {
        ...
        const currentValue = currentProgress.gamesProgress?.sundew_vally || 0;
        await updateUserData({
          progress: {
            ...currentProgress,
            gamesProgress: {
              ...currentProgress.gamesProgress,
              sundew_vally: Math.min(100, currentValue + 1),
            },
          },
        });
      }
