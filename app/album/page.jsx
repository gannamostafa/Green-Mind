...
export default function AlbumPage() {
  ...
  useEffect(() => {
    ...
    try {
      if (userData && userData.progress && Array.isArray(userData.progress.album)) {
        setAlbum(userData.progress.album);
  ...
  const handleLogout = async () => {
    await signOut(auth);
    // Do not clear localStorage so album persists as it's stored in Firestore
    router.push("/login");
  };
              ...
              {filteredAlbum.map((item) => (
                ...
                >
                  {typeof item.image === "string" && (item.image.startsWith("http:") || item.image.startsWith("https:")) ? (
                    ...
                    // Fallback to standard img for blob/data URLs or unknown formats
                    <img src={item.image} width={200} height={150} alt="plant" className="rounded-xl mx-auto" />
                  )}
