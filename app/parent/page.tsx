...
// Firebase Firestore helper imports
import { doc, getDoc, setDoc } from "firebase/firestore";
import { db } from "@/lib/firebase";

// Default progress template for a brand‑new child
...
// Ensure child document exists; create with defaults if missing
const ensureChildDoc = async (uid: string, childId: string) => {
  const childRef = doc(db, "users", uid, "children", childId);
  const snap = await getDoc(childRef);
  if (!snap.exists()) {
    await setDoc(childRef, { progress: defaultProgress });
  }
};
...
/* ===============================================================
   EDIT PROFILE POPUP
=============================================================== */
import EditProfilePopup from "./edit-profile";
...
export default function ParentDashboard() {
  ...
  const weeklyData = (progress.weeklyProgress || [0,0,0,0,0,0,0]).map(
    (val: number, i: number) => ({ name: daysOfWeek[i] || `Day ${i + 1}`, progress: val })
  );

  const displayQuizScores = [
    { label: "Quiz 1", value: progress.quizScores?.video1 || 0 },
    { label: "Quiz 2", value: progress.quizScores?.video2 || 0 },
    { label: "Quiz 3", value: progress.quizScores?.video3 || 0 },
    { label: "Quiz 4", value: progress.quizScores?.video4 || 0 },
    { label: "Quiz 5", value: progress.quizScores?.video5 || 0 },
    { label: "Quiz 6", value: progress.quizScores?.video6 || 0 },
  ];

  const solvedQuizzes = Object.values(progress.quizScores || {}).map((val: any) => Number(val));
  const avgQuizScore = solvedQuizzes.length > 0
    ? Math.round(solvedQuizzes.reduce((a, b) => a + b, 0) / solvedQuizzes.length)
    : 0;
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link href="/team" className="block">
              <div className="px-4 py-3 rounded-2xl flex items-center gap-3 text-white hover:bg-white/20 transition font-medium cursor-pointer">
                <Image src="/SCreen/Union.png" width={24} height={24} alt="team" className="brightness-0 invert" />
                <span>Our Team 👥</span>
              </div>
            </Link>
          </div>
