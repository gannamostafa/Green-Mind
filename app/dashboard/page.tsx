...
import EditProfilePopup from "@/app/parent/edit-profile";
...
export default function Dashboard() {
  ...
  const progress = {
    ...
    gamesProgress: {
      ...
      sundew_vally: userData?.progress?.gamesProgress?.sundew_vally || 0,
    },
  ...
  const progressPercent = progress.treeLevel >= 5 ? 100 : currentLevelXp;
  ...
  // Scan localStorage for game completions and sync to Firestore
  useEffect(() => {
    ...
    const syncProgress = async () => {
      ...
      // Count Puzzle completions (have levelXStars set)
      for (let i = 1; i <= 10; i++) {
        if (localStorage.getItem(`level${i}Completed`) === "true" && localStorage.getItem(`level${i}Stars`)) {
          puzzleCompleted++;
        }
      }

      // Count Memory completions
      for (let i = 1; i <= 10; i++) {
        if (localStorage.getItem(`memory_level${i}Completed`) === "true") {
          memoryCompleted++;
        }
      }
      ...
      const isPuzzleLocked = !currentProgress.lessonsCompleted?.includes("video1");
      const isMemoryLocked = !currentProgress.lessonsCompleted?.includes("video2");
      const isFarmLocked = !currentProgress.lessonsCompleted?.includes("video3");

      const puzzlePercent = isPuzzleLocked ? 0 : Math.round((puzzleCompleted / 10) * 100);
      const memoryPercent = isMemoryLocked ? 0 : Math.round((memoryCompleted / 10) * 100);

      // Matching progress from Sundew Valley save state
      let matchingPercent = 0;
      if (!isFarmLocked) {
        const sundewSave = localStorage.getItem("sundew_valley_save");
        if (sundewSave) {
          try {
            const data = JSON.parse(sundewSave);
            const inventoryCount = Object.keys(data.player?.inventory || {}).length;
            const questsCompleted = data.world?.quests?.completedQuests?.length || 0;
            matchingPercent = Math.min(100, Math.round(((questsCompleted + inventoryCount) / 10) * 100));
          } catch (e) {}
        }
      }

      const dbPuzzle = currentProgress.gamesProgress?.puzzle || 0;
      const dbMemory = currentProgress.gamesProgress?.memory || 0;
      const dbSundewVally = currentProgress.gamesProgress?.sundew_vally || 0;

      const hasChanges =
        puzzlePercent > dbPuzzle ||
        memoryPercent > dbMemory ||
        matchingPercent > dbSundewVally ||
        (isPuzzleLocked && dbPuzzle > 0) ||
        (isMemoryLocked && dbMemory > 0) ||
        (isFarmLocked && dbSundewVally > 0);

      if (hasChanges) {
        // Award 15 XP for every newly completed level!
        const completedLevelsInDb = Math.round((dbPuzzle + dbMemory) / 10);
        const newCompletions = Math.max(0, (puzzleCompleted + memoryCompleted) - completedLevelsInDb);
        const xpGained = newCompletions * 15;
        ...
        await updateUserData({
          progress: {
            ...
            treeLevel: newTreeLevel,
            gamesProgress: {
              puzzle: isPuzzleLocked ? 0 : Math.max(dbPuzzle, puzzlePercent),
              memory: isMemoryLocked ? 0 : Math.max(dbMemory, memoryPercent),
              sundew_vally: isFarmLocked ? 0 : Math.max(dbSundewVally, matchingPercent)
            }
          }
        });
        console.log("Synced game progress from dashboard mount!", { puzzlePercent, memoryPercent, matchingPercent });
      }
    };

    syncProgress().catch((err) => console.error("Error syncing games progress:", err));
...
<div className="rounded-[40px]  flex flex-col md:flex-row items-center justify-between mb-2 relative overflow-hidden">
  ...
  <div className="z-10 flex-1 text-left">
    ...
    <div className="w-full max-w-md bg-gray-200/80 border border-gray-300/30 h-4 rounded-full mb-1.5 overflow-hidden shadow-inner relative flex items-center">
      <div
    ...
    <p className="text-xs text-gray-500 font-bold mb-6">
      {progress.treeLevel >= 5
        ? "⭐ Max Level Reached! You are an Eco Legend! 🌳"
        : `🌱 ${currentLevelXp}/100 XP to Tree Level ${progress.treeLevel + 1}`}
    </p>
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
    ...
    <div className="bg-white rounded-[40px] p-1 shadow-md h-[320px] flex flex-col items-center justify-between border border-gray-100 overflow-hidden relative">
      ...
      <div className="relative z-10 flex flex-col items-center h-full w-full">
        ...
        {/* Level */}
        <div className="mb-1">
          <div className="px-4 py-0.5  rounded-full bg-green-200/70 backdrop-blur-md border-2 border-green-700 text-green-800 font-bold text-2xl shadow-lg">
    ...
    <div className="bg-white rounded-[40px] p-6 shadow-md h-[320px] border border-gray-100 flex flex-col justify-between">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex justify-between text-sm font-semibold text-gray-500 mb-2">
            <span>Sundew Valley</span>
            <span>{progress.gamesProgress.sundew_vally}%</span>
          </div>
          ...
          <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
            <div
              className="h-full bg-cyan-500 rounded-full"
              style={{
                width: `${progress.gamesProgress.sundew_vally}%`,
              }}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-10 items-stretch text-left">
            ...
            <TodaysActivity
              ...
              timeData={[
                ...
                { name: "G", value: Math.min(Math.round((progress.gamesProgress.puzzle + progress.gamesProgress.memory + progress.gamesProgress.sundew_vally) / 15), 40) },
              ]}
              timeColors={timeColors}
              lessonCount={progress.lessonsCompleted.length}
              gameAvg={Math.round((progress.gamesProgress.puzzle + progress.gamesProgress.memory + progress.gamesProgress.sundew_vally) / 3)}
            />

            <TrophiesSection
              completedCount={progress.lessonsCompleted.length}
              weeklyProgress={progress.weeklyProgress}
              sunCount={userData?.progress?.sunCount || 0}
            />
          </div>
        </main>
      </div> 
...
function TrophiesSection({ completedCount, weeklyProgress, sunCount }: any) {
  // Calculate active days in the week
  const activeDaysCount = (weeklyProgress || []).filter((v: number) => v > 0).length;
  ...
  const trophies = [
    ...
    },
    {
      ...
      earned: activeDaysCount >= 3,
      progress: `${Math.min(3, activeDaysCount)}/3`,
    },
  ];
    ...
    <div className="trophies-playful-card p-5 h-[320px] flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-3 flex-1">
        {trophies.map((trophy, i) => (
          <div
            key={i}
            className={`relative trophy-item-playful bg-white/85 backdrop-blur-sm rounded-[24px] p-3 flex flex-col items-center justify-center border transition-all duration-300 shadow-sm ${
              trophy.earned 
                ? "border-sky-300/60 bg-gradient-to-br from-white to-sky-50/30" 
                : "border-gray-200/50 bg-gray-50/40 opacity-75 filter grayscale"
            }`}
          >
            {/* Checkmark or Lock Ribbon */}
            {trophy.earned ? (
              ...
              <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-gray-200 flex items-center justify-center text-gray-400 font-extrabold text-[10px] border border-white">
                🔒
              </div>
            )}
            ...
            <Image
              ...
              width={50}
              height={50}
              className={`object-contain mb-1.5 drop-shadow-md ${trophy.earned ? "animate-pulse-slow" : ""}`}
            />

            {/* Trophy Name */}
            <h4 className={`font-extrabold text-[11px] whitespace-nowrap tracking-tight ${trophy.earned ? "text-sky-700" : "text-gray-500"}`}>
              {trophy.name}
            </h4>

            {/* Achievement Goal */}
            <p className="text-gray-400 text-[9px] whitespace-nowrap mt-0.5 font-semibold">
              {trophy.desc}
            </p>

            {/* Progress Badge */}
            <span className={`text-[9px] font-black mt-1 px-2 py-0.5 rounded-full border ${
              trophy.earned 
            ...
            {/* Award Points */}
            <div className={`mt-2 px-3 py-0.5 rounded-full border font-extrabold text-[10px] shadow-inner ${
              trophy.earned 
                ? "border-sky-200 text-sky-600 bg-sky-50/70" 
